...
const Dashboard = () => {
  ...
  useEffect(() => {
    const perangkatRef = ref(database, "/Volt_Meter");
  ...
  const handlePerangkatClick = (perangkat) => {
    navigate(`/perangkat_tegangan/${perangkat}`);
  };

  return (
    <div className="flex min-h-screen bg-gradient-to-b from-[#f6f9ff] to-[#eaf0ff] text-gray-900 ml-0 md:ml-64">
      <Sidebar />
      <main className="flex-1 p-4 md:p-6 lg:p-8">
        ...
        <div className="flex items-center justify-between mb-2">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center text-gray-800 hover:text-blue-100"
          >
          ...
          <div className="w-[50px]">{/* Spacer untuk meratakan header */}</div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-10 max-w-5xl mx-auto">
          {perangkatList.map((perangkat) => (
            <button
              ...
              className="py-6 bg-white hover:bg-blue-100 text-black font-semibold rounded-xl w-full text-center border border-blue-200 shadow"
            >
              {perangkat}
            </button>
          ))}
